...
const { useState, useEffect, useRef } = React;
    ...
    function DashboardView({ data, taches, finance, onNewClient, setView }) {
      ...
      const tachesAujourdhuiDemain = tachesEnCours.filter(t => !tachesUrgentes.includes(t) && t.deadline && (t.deadline === todayStr || t.deadline === tomorrowStr));
      ...
      function diffLabel(deadline) {
        if (!deadline) return null;
        const diff = Math.round((new Date(deadline + 'T00:00:00') - today) / 86400000);
        if (diff < 0) return { txt: `J+${Math.abs(diff)} retard`, color: 'var(--danger)' };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="metrics-grid">
            ...
            <div className="metric-card accent-warn" onClick={() => setView('taches')} style={{ cursor: 'pointer' }}>
              ...
              <div className="metric-sub">{tachesUrgentes.length} urgentes</div>
            </div>
            <div className="metric-card" onClick={() => setView('devis')} style={{ cursor: 'pointer' }}>
              <div className="metric-label">Devis envoyés</div>
            ...
            <div className="metric-card accent-green" onClick={() => setView('devis')} style={{ cursor: 'pointer' }}>
              ...
              <div className="metric-value" style={{ fontSize: 20 }}>{((finance?.factures?.ca_encaisse) || 0).toLocaleString('fr-FR')} €</div>
            </div>
           </div>
           ...
           <div className="grid-2">
             ...
             <div className="card" style={{ borderTop: '4px solid var(--danger)' }}>
               <div className="card-header"><div className="card-title" style={{ color: 'var(--danger)' }}>🔥 URGENCES & RETARDS</div></div>
               
               {tachesUrgentes.length === 0 && facturesEnRetard.length === 0 && (
                 <div className="empty" style={{ padding: '12px 0' }}><div className="empty-text">Aucune urgence. 🧘</div></div>
               )}

               {tachesUrgentes.map(t => (
                 <div key={t.id} style={{ display: 'flex', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    <div style={{ flex: 1 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                       {t.priorite === 'URGENT' && <span style={{ background: 'var(--danger)', color: 'white', fontSize: 10, padding: '2px 6px', borderRadius: 4 }}>URGENT</span>}
                       {t.deadline && <div style={{ fontSize: 10, color: 'var(--danger)', marginTop: 4 }}>{diffLabel(t.deadline)?.txt}</div>}
                    </div>
                 </div>
               ))}

               {facturesEnRetard.map(f => (
                 <div key={f.id} style={{ display: 'flex', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                     <div style={{ flex: 1 }}>
                       <div style={{ fontSize: 13, fontWeight: 500 }}>Facture impayée : {f.numero}</div>
                       <div style={{ fontSize: 11, color: 'var(--danger)' }}>{f.client_nom}</div>
                     </div>
                     <div style={{ textAlign: 'right', fontWeight: 700, color: 'var(--danger)' }}>
                       {Number(f.total_ht).toLocaleString('fr-FR')} €
                     </div>
                 </div>
               ))}
             </div>
             ...
             <div className="card" style={{ borderTop: '4px solid var(--warn)' }}>
                <div className="card-header"><div className="card-title">📅 AUJOURD'HUI & DEMAIN</div></div>
                {tachesAujourdhuiDemain.length === 0 ? (
                  <div className="empty" style={{ padding: '12px 0' }}><div className="empty-text">Rien de prévu pour ces 48h.</div></div>
                  ...
                  <div key={t.id} style={{ display: 'flex', gap: 8, padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <div style={{ textAlign: 'right', fontSize: 11, fontWeight: 600, color: 'var(--warn)' }}>
                       {diffLabel(t.deadline)?.txt}
                    </div>
                  </div>
                ))}
             </div>
             ...
             {/* EN ATTENTE */}
             <div className="card" style={{ gridColumn: '1 / -1', borderTop: '4px solid var(--pk-blue)' }}>
               <div className="card-header"><div className="card-title">⏳ SUIVI (Devis & Factures envoyés)</div></div>
               
               <div className="grid-2">
                 <div>
                   <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text3)', marginBottom: 8, textTransform: 'uppercase' }}>Devis en attente de signature</div>
                   {devisEnvoyes.length === 0 ? <div className="empty-text" style={{ fontSize: 12 }}>Aucun devis en attente.</div> : devisEnvoyes.map(d => (
                     <div key={d.id} style={{ padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
                        <div style={{ fontWeight: 600 }}>{d.client_nom}</div>
                        <div style={{ color: 'var(--text2)' }}>{d.numero} — {Number(d.total_ht).toLocaleString()}€</div>
                     </div>
                   ))}
                 </div>
                 <div>
                   <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text3)', marginBottom: 8, textTransform: 'uppercase' }}>Factures en attente de paiement</div>
                   {facturesEnvoyees.length === 0 ? <div className="empty-text" style={{ fontSize: 12 }}>Aucune facture en attente.</div> : facturesEnvoyees.map(f => (
                     <div key={f.id} style={{ padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
                        <div style={{ fontWeight: 600 }}>{f.client_nom}</div>
                        <div style={{ color: 'var(--text2)' }}>{f.numero} — {Number(f.total_ht).toLocaleString()}€</div>
                        <div style={{ color: 'var(--warn)', fontSize: 10 }}>Échéance : {f.date_echeance}</div>
                     </div>
                   ))}
                 </div>
               </div>
          ...
          {/* Bloc subventions dashboard */}
          {subventions.length > 0 && (
            <div className="grid-2">
              <div className="card">
                <div className="card-header">
                  <div className="card-title">🏛 Subventions</div>
                  <button className="btn" style={{ fontSize: 11 }} onClick={() => setView('subventions')}>Voir tout →</button>
                </div>
                <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                  <div style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--success-light)', borderRadius: 8 }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--success)' }}>{caSubvObtenu.toLocaleString('fr-FR')} €</div>
                    <div style={{ fontSize: 10, color: 'var(--success)' }}>Obtenus / Versés</div>
                  </div>
                  <div style={{ flex: 1, textAlign: 'center', padding: '8px', background: 'var(--warn-light)', borderRadius: 8 }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--warn)' }}>{subvEnCours.length}</div>
                    <div style={{ fontSize: 10, color: 'var(--warn)' }}>En cours</div>
                  </div>
                </div>
                {(() => {
                  ...
                  const tachesSubv = subventions
                    ...
                    .sort((a,b) => a.deadline.localeCompare(b.deadline))
                    .slice(0, 4);
                  if (!tachesSubv.length) return <div style={{ fontSize: 12, color: 'var(--text3)', textAlign: 'center', padding: '8px 0' }}>Aucune tâche de suivi active</div>;
                  return tachesSubv.map(t => {
                    const diff = Math.round((new Date(t.deadline+'T00:00:00') - new Date()) / 86400000);
                    return (
                      <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
                        <div>
                          <div style={{ fontWeight: 500 }}>{t.titre}</div>
                          <div style={{ fontSize: 10, color: 'var(--text3)' }}>{t.subNom} · {t.clientNom}</div>
                        </div>
                        <span style={{ fontWeight: 700, fontSize: 11, color: diff < 0 ? 'var(--danger)' : diff <= 7 ? 'var(--warn)' : 'var(--text3)' }}>
                          {diff < 0 ? `J+${Math.abs(diff)}` : `J-${diff}`}
                        </span>
                      </div>
                    );
                  });
              ...
              <div className="card">
                <div className="card-header"><div className="card-title">⏰ Deadlines subventions</div></div>
                {subvDeadlines.length === 0
                  ? <div style={{ fontSize: 12, color: 'var(--text3)', textAlign: 'center', padding: '8px 0' }}>Aucune deadline à venir</div>
                  : subvDeadlines.map(s => {
                    const diff = Math.round((new Date(s.deadline+'T00:00:00') - new Date()) / 86400000);
                    return (
                      <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: 'var(--pk-blue)' }}>{s.client_nom}</div>
                        </div>
                        <span style={{ fontWeight: 700, fontSize: 12, color: diff <= 3 ? 'var(--danger)' : diff <= 14 ? 'var(--warn)' : 'var(--text3)', flexShrink: 0 }}>
                          {diff <= 0 ? 'Dépassé' : `J-${diff}`}
                        </span>
                      </div>
                    );
                  })
...
<div class="conditions">
  <strong>Conditions :</strong> Validité ${d.validite_jours || 30} jours · Délai de paiement 30 jours nets · Pénalités de retard : 3× le taux légal · TVA non applicable — Art. 293B du CGI
    ...
    const PIECES_STANDARD_SUB = [
        ...
        { id: "budget_prev", label: "Budget prévisionnel du projet", required: true },
